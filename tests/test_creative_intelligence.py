...
from devintel.modules.creative import (
    AssetKind, CreativeBrief, CreativeConsistency, CreativeIntelligence, CreativePipeline,
    CreativeProvider, CreativeProviderRegistry, CreativeStatus, brief_digest, plan_digest,
)

def make_brief():
    return CreativeBrief("Explain a complex idea clearly", "general learners", AssetKind.VIDEO, ("respect copyright", "protect privacy"), ("clear", "useful"), ("evidence:claim-1",))
...
def test_ideation_and_variations_are_bounded_and_distinct():
    engine = CreativeIntelligence(); concepts = engine.ideate(make_brief(), count=4); variants = engine.variations(concepts[0], count=4)
    assert len(concepts) == 4 and len({c.title for c in concepts}) == 4; assert len(variants) == 4 and len({c.title for c in variants}) == 4
...
def test_custom_weak_criterion_can_be_repaired_but_remains_truthful():
    engine = CreativeIntelligence(); plan = engine.plan(make_brief()); evaluation = engine.critique(plan, ("unknown",)); assert evaluation.status is CreativeStatus.NEEDS_REVIEW and evaluation.defects
    assert engine.revise(plan, evaluation).status is CreativeStatus.READY_FOR_CREATION

def test_consistency_is_validated_and_bound_to_plan_digest():
    consistency = CreativeConsistency("cinematic", "hero-1", "forest", "1920x1080", 24, "16:9", "stable", ("color-grade",))
    plan = CreativeIntelligence().plan(make_brief(), consistency=consistency)
    assert plan.consistency == consistency and plan_digest(plan) != CreativeIntelligence().plan(make_brief()).brief_digest
    with pytest.raises(ValueError): CreativeConsistency(f"x" * 16385)
...
def test_provider_selection_fails_closed_and_prefers_trust_with_fallbacks():
    registry = CreativeProviderRegistry([CreativeProvider("low", "Low", (AssetKind.VIDEO,), approved=True, trust_score=.4), CreativeProvider("high", "High", (AssetKind.VIDEO,), approved=True, trust_score=.9), CreativeProvider("blocked", "Blocked", (AssetKind.VIDEO,), approved=False, trust_score=1.0)])
    selection = registry.select(AssetKind.VIDEO); assert selection.provider_id == "high" and selection.fallback_provider_ids == ("low",); assert registry.select(AssetKind.IMAGE).provider_id is None

def test_pipeline_binds_artifact_to_plan_and_has_no_external_side_effects():
    provider = CreativeProvider("local", "Local", (AssetKind.VIDEO,), approved=True, trust_score=.8); result = CreativePipeline(providers=CreativeProviderRegistry([provider])).run(make_brief())
    assert result.evaluation.status is CreativeStatus.ACCEPTED and result.provider.provider_id == "local"; assert result.artifact.plan_digest == plan_digest(result.plan); assert result.artifact.artifact_digest
