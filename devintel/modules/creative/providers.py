...
class CreativeProviderRegistry:
    ...
    def candidates(self, asset_kind: AssetKind) -> tuple[CreativeProvider, ...]:
        result = [p for p in self._providers.values() if p.available and p.approved and asset_kind in p.asset_kinds]
        result.sort(key=lambda p: (-p.trust_score, p.cost_class, p.provider_id))
        return tuple(result)
    def select(self, asset_kind: AssetKind) -> ProviderSelection:
        candidates = self.candidates(asset_kind)
        if not candidates: return ProviderSelection(asset_kind, None, "no approved available provider", 0)
        return ProviderSelection(asset_kind, candidates[0].provider_id, "highest-trust approved provider; remaining candidates are bounded fallbacks", len(candidates), tuple(p.provider_id for p in candidates[1:]))
