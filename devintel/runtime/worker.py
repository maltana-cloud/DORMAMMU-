...
class RuntimeJobStore:
    ...
    def enqueue(self, scope_id: str, action: str, payload: dict[str, Any], *, job_id: str | None = None) -> RuntimeJob:
        ...
        now = _now()
        job = RuntimeJob(job_id or uuid4().hex, scope_id, action, dict(payload), "queued", 0, now, now, "")
        self._db.execute(
            "INSERT INTO runtime_jobs VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (job.job_id, job.scope_id, job.action, json.dumps(job.payload, sort_keys=True), job.status, job.attempts, job.created_at, job.updated_at, job.last_error),
        )
        self._db.commit()
        return job

    def get(self, job_id: str) -> RuntimeJob | None:
        row = self._db.execute("SELECT * FROM runtime_jobs WHERE job_id = ?", (job_id,)).fetchone()
        return self._from_row(row) if row else None

    def claim_next(self) -> RuntimeJob | None:
        row = self._db.execute(
            "SELECT * FROM runtime_jobs WHERE status = 'queued' ORDER BY created_at, job_id LIMIT 1"
        ).fetchone()
        if row is None:
            return None
        now = _now()
        updated = self._db.execute(
            "UPDATE runtime_jobs SET status='running', attempts=attempts+1, updated_at=? WHERE job_id=? AND status='queued'",
            (now, row[0]),
        )
        self._db.commit()
    ...
    def complete(self, job_id: str) -> RuntimeJob:
    ...
    def recover_running(self) -> tuple[RuntimeJob, ...]:
        rows = self._db.execute("SELECT * FROM runtime_jobs WHERE status='running' ORDER BY created_at, job_id").fetchall()
        now = _now()
        self._db.execute("UPDATE runtime_jobs SET status='queued', updated_at=? WHERE status='running'", (now,))
        self._db.commit()
    ...
    def history(self, scope_id: str | None = None) -> tuple[RuntimeJob, ...]:
        if scope_id is None:
            rows = self._db.execute("SELECT * FROM runtime_jobs ORDER BY created_at, job_id").fetchall()
        else:
            rows = self._db.execute("SELECT * FROM runtime_jobs WHERE scope_id=? ORDER BY created_at, job_id", (scope_id,)).fetchall()
    ...
    def _transition(self, job_id: str, status: str, error: str) -> RuntimeJob:
        now = _now()
        updated = self._db.execute(
            "UPDATE runtime_jobs SET status=?, updated_at=?, last_error=? WHERE job_id=? AND status='running'",
    ...
    @staticmethod
    def _from_row(row: tuple[Any, ...]) -> RuntimeJob:
        return RuntimeJob(row[0], row[1], row[2], json.loads(row[3]), row[4], row[5], row[6], row[7], row[8])


class RuntimeWorker:
    """Execute at most a caller-defined number of durable jobs per invocation."""

    def __init__(self, store: RuntimeJobStore, dispatch: Callable[[RuntimeJob], Any]) -> None:
        self.store = store
    ...
    def run_once(self, *, max_jobs: int = 1) -> tuple[RuntimeJob, ...]:
        ...
        for _ in range(max_jobs):
            ...
            try:
                ...
                completed.append(self.store.fail(job.job_id, type(exc).__name__))
            else:
                completed.append(self.store.complete(job.job_id))
