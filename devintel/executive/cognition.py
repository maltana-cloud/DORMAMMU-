...
class CoreCognition:
    ...
    def from_synthesis(self, scope_id: str, synthesis: SynthesisResult, *, known_capabilities: Sequence[str] = (), known_resources: Sequence[str] = (), known_limits: Sequence[str] = ()) -> CognitiveState:
        if not isinstance(scope_id, str) or not scope_id.strip():
            raise ValueError("scope_id is required")
        if not isinstance(synthesis, SynthesisResult):
            raise TypeError("synthesis must be a SynthesisResult")
        signals = synthesis.signals[:_MAX_FACTS + _MAX_HYPOTHESES]
        facts = tuple(CognitiveFact(s.statement, s.confidence, s.evidence_urls, scope_id) for s in signals if not s.uncertain and s.confidence >= 0.5)[:_MAX_FACTS]
        hypotheses = tuple(CognitiveHypothesis(s.statement, s.confidence, s.evidence_urls) for s in signals if s.uncertain or s.confidence < 0.5)[:_MAX_HYPOTHESES]
        questions = ("Resolve contradictory or insufficient evidence before high-confidence use.",) if synthesis.contradictions or not facts else ()
        self_model = CognitiveSelfModel(tuple(dict.fromkeys(str(x) for x in known_capabilities))[:_MAX_HISTORY], tuple(dict.fromkeys(str(x) for x in known_resources))[:_MAX_HISTORY], tuple(dict.fromkeys(str(x) for x in known_limits))[:_MAX_HISTORY])
    ...
    def compare(self, left: CognitiveState, right: CognitiveState) -> tuple[str, ...]:
        ...
        return tuple(sorted({f.statement for f in right.facts} - {f.statement for f in left.facts}))
