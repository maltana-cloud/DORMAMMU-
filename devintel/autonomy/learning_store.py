...
class LearningStore:
    def __init__(self, path: str = ":memory:") -> None:
        ...
        self.connection.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_learning_proposal_identity ON learning_proposals(scope_id,basis_cycle_ids,adjustment,expected_delta,confidence,reversible,subject_id)")
    ...
    def outcomes(self, scope_id: str, limit: int = 100) -> tuple[OutcomeEvidence,...]:
        ...
        rows=self.connection.execute("SELECT cycle_id,scope_id,success,verified,metric,observation,recorded_at,subject_id FROM outcome_evidence WHERE scope_id=? ORDER BY recorded_at DESC,cycle_id DESC LIMIT ?",(scope_id,limit)).fetchall()
    ...
    def record_proposal(self, proposal: LearningProposal) -> None:
        self.connection.execute("INSERT OR IGNORE INTO learning_proposals(scope_id,basis_cycle_ids,adjustment,expected_delta,confidence,reversible,subject_id) VALUES (?,?,?,?,?,?,?)",(proposal.scope_id,json.dumps(proposal.basis_cycle_ids),proposal.adjustment,proposal.expected_delta,proposal.confidence,int(proposal.reversible),proposal.subject_id))
        self.connection.commit()
