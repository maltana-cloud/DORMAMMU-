...
class MissionStore:
    ...
    def _ensure_lease_columns(self)->None:
        columns={r[1] for r in self._db.execute("PRAGMA table_info(missions)").fetchall()}
        if "lease_worker_id" not in columns:self._db.execute("ALTER TABLE missions ADD COLUMN lease_worker_id TEXT")
        if "lease_expires_at" not in columns:self._db.execute("ALTER TABLE missions ADD COLUMN lease_expires_at REAL")
    def create(self,scope_id:str,objective:str,total_steps:int,*,max_attempts:int=3,mission_id:str|None=None,now:float=0.0)->Mission:
        ...
        t=_now();m=Mission(mission_id or uuid4().hex,scope_id.strip(),objective.strip(),total_steps,0,MissionStatus.QUEUED,0,max_attempts,now,t,t)
    ...
    def record_step(self,mission_id:str,step_index:int,*,verified:bool,message:str="",data:Mapping[str,object]|None=None)->MissionStepRecord:
        ...
        r=self._db.execute("SELECT completed FROM mission_steps WHERE mission_id=? AND step_index=?",(mission_id,step_index)).fetchone()
    ...
    def release_lease(self,mission_id:str,*,worker_id:str,now:float)->Mission:
        if not worker_id.strip() or now<0:raise ValueError("invalid lease release inputs")
        u=self._db.execute("UPDATE missions SET lease_worker_id=NULL,lease_expires_at=NULL,updated_at=? WHERE mission_id=? AND lease_worker_id=? AND status='running'",(_now(),mission_id,worker_id.strip()))
        if u.rowcount!=1:raise ValueError("active lease is not owned by worker")
    ...
    def _require_running(self,mission_id:str)->Mission:
        ...
        return m
    @staticmethod
    def _from_row(r:sqlite3.Row)->Mission:return Mission(r["mission_id"],r["scope_id"],r["objective"],r["total_steps"],r["current_step"],MissionStatus(r["status"]),r["attempts"],r["max_attempts"],r["next_run_at"],r["created_at"],r["updated_at"],r["last_error"])
