...
class MissionStep:
    ...
    def __post_init__(self) -> None:
        if not self.step_id.strip() or not self.name.strip():
            raise ValueError("step_id and name are required")
        if self.payload is None:
            object.__setattr__(self, "payload", {})
        if len(self.payload or {}) > 64:
...
class MissionStore:
    ...
    def __init__(self, path: str = ":memory:") -> None:
        ...
        self._db.execute("""CREATE TABLE IF NOT EXISTS mission_steps (
            ...
            PRIMARY KEY (mission_id, step_index), UNIQUE (mission_id, step_id),
    ...
    def create(self, scope_id: str, objective: str, total_steps: int, *, max_attempts: int = 3, mission_id: str | None = None, now: float = 0.0) -> Mission:
        ...
        if total_steps <= 0 or max_attempts <= 0 or now < 0:
            raise ValueError("invalid mission bounds")
        created = _now()
    ...
    def define_steps(self, mission_id: str, steps: tuple[MissionStep, ...]) -> tuple[MissionStepRecord, ...]:
        ...
        if len(steps) != mission.total_steps or len({s.step_id for s in steps}) != len(steps):
            raise ValueError("step count must equal total_steps and step ids must be unique")
        if self._db.execute("SELECT COUNT(*) FROM mission_steps WHERE mission_id=?", (mission_id,)).fetchone()[0]:
            raise ValueError("mission steps are already defined")
        recorded = _now()
        self._db.executemany("INSERT INTO mission_steps VALUES (?,?,?,?,?,?,?,?,?,?)", [(mission_id, i, s.step_id, s.name, json.dumps(dict(s.payload or {}), sort_keys=True), 0, 0, "", "{}", recorded) for i, s in enumerate(steps)])
    ...
    def steps(self, mission_id: str) -> tuple[MissionStepRecord, ...]:
        ...
        return tuple(MissionStepRecord(r["mission_id"], r["step_index"], MissionStep(r["step_id"], r["name"], json.loads(r["payload"])), bool(r["completed"]), bool(r["verified"]), r["message"], json.loads(r["data"]), r["recorded_at"]) for r in rows)
    ...
    def record_step(self, mission_id: str, step_index: int, *, verified: bool, message: str = "", data: Mapping[str, object] | None = None) -> MissionStepRecord:
        ...
        if not 0 <= step_index < mission.total_steps or len(message) > 2000:
            raise ValueError("invalid step record")
        ...
        if row is None or row["completed"]:
            raise ValueError("mission step is undefined or already completed")
        self._db.execute("UPDATE mission_steps SET completed=1, verified=?, message=?, data=?, recorded_at=? WHERE mission_id=? AND step_index=?", (int(verified), message, json.dumps(dict(data or {}), sort_keys=True), _now(), mission_id, step_index))
    ...
    def claim_due(self, *, now: float, scope_id: str | None = None, worker_id: str | None = None, lease_ttl_seconds: float | None = None) -> Mission | None:
        ...
        query = "SELECT mission_id FROM missions WHERE status IN ('queued','running') AND next_run_at<=? AND (lease_expires_at IS NULL OR lease_expires_at<=?)"; args: list[Any] = [now, now]
        if scope_id is not None: query += " AND scope_id=?"; args.append(scope_id)
        row = self._db.execute(query + " ORDER BY next_run_at, created_at, mission_id LIMIT 1", args).fetchone()
    ...
    def checkpoint(self, mission_id: str, *, current_step: int, now: float, worker_id: str | None = None) -> Mission:
        ...
        if current_step and self._db.execute("SELECT COUNT(*) FROM mission_steps WHERE mission_id=? AND completed=1 AND verified=1 AND step_index<?", (mission_id, current_step)).fetchone()[0] != current_step: raise ValueError("checkpoint requires verified prior steps")
        status = MissionStatus.SUCCEEDED if current_step == mission.total_steps else MissionStatus.QUEUED
        if worker_id is None:
            sql = "UPDATE missions SET status=?, current_step=?, next_run_at=?, updated_at=?, last_error='', lease_worker_id=NULL, lease_expires_at=NULL WHERE mission_id=? AND status='running'"; args = (status, current_step, now, _now(), mission_id)
        else:
    ...
    def fail(self, mission_id: str, error: str, *, now: float, backoff_seconds: float = 0.0, worker_id: str | None = None) -> Mission:
        ...
        owner = " AND lease_worker_id=?" if worker_id is not None else ""; args: list[Any] = [status, next_run, _now(), error[:2000], mission_id]
    ...
    def resume(self, mission_id: str, *, now: float) -> Mission:
        mission = self._require(mission_id)
        if mission.status is not MissionStatus.PAUSED or now < 0: raise ValueError("only paused missions can be resumed")
