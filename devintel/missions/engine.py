...
class MissionRunner:
    ...
    ``step`` receives the current mission and must return successfully when the
    current step has been completed. It has no implicit authority to run again;
    the caller controls invocation frequency and the mission's finite bounds.
    """
        ...
        *,
        ...
        for _ in range(policy.max_steps):
            if policy.max_duration_seconds is not None and self.clock() - started >= policy.max_duration_seconds:
                break
            mission = self.store.claim_due(now=now, scope_id=scope_id)
            if mission is None:
                break
            try:
                step(mission)
            except Exception as exc:
                results.append(self.store.fail(mission.mission_id, type(exc).__name__, now=now, backoff_seconds=policy.retry_backoff_seconds))
                continue
            results.append(self.store.checkpoint(mission.mission_id, current_step=mission.current_step + 1, now=now))
