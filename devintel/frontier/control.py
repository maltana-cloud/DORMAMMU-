...
class FrontierResourceBudget:
    ...
    def __post_init__(self) -> None:
        if not self.resource_id.strip() or not self.scope_id.strip():
            raise ValueError("resource_id and scope_id are required")
...
class FrontierJobStore:
    ...
    def recover_expired(self, *, now: float) -> int:
        ...
        result = self._db.execute(
            "UPDATE frontier_jobs SET state='queued', worker_id=NULL, lease_expires_at=NULL "
    ...
    def reserve(self, *, resource_id: str, quantity: float, scope_id: str) -> bool:
        ...
        result = self._db.execute(
            ...
            (quantity, resource_id, scope_id, quantity),
        )
    ...
    def record_reflection(self, reflection: FrontierReflection, *, recorded_at: float = 0.0) -> str:
        ...
        rid = hashlib.sha256(
            json.dumps((reflection.job_id, reflection.outcome, reflection.evidence,
                        reflection.lesson, reflection.uncertainty), sort_keys=True).encode()
        ).hexdigest()
        self._db.execute(
    ...
    def reflections(self, job_id: str, limit: int = 20) -> tuple[FrontierReflection, ...]:
        ...
        rows = self._db.execute(
            ...
            "WHERE job_id=? ORDER BY recorded_at DESC LIMIT ?", (job_id, limit)
        ).fetchall()
