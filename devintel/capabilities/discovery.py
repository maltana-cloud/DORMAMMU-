...
class DefaultEvaluator:
    ...
    def evaluate(self, requirement: CapabilityRequirement, candidate: CapabilityDescriptor) -> Evaluation:
        reasons: list[str] = []
        trust = candidate.provider.strip() != "unknown"
        security = "unsafe" not in {x.lower() for x in candidate.metadata.values()}
        compatibility = set(requirement.required_interfaces).issubset(candidate.interfaces)
        performance = candidate.metadata.get("performance", "unknown").lower() not in {"poor", "failed"}
        license_ok = not requirement.required_license or candidate.license == requirement.required_license
        cost_ok = (requirement.max_cost is None or candidate.cost <= requirement.max_cost) and (self.policy.allow_paid or candidate.cost == 0)
        permission_ok = not self.policy.require_explicit_permission or "approved" in candidate.permissions
