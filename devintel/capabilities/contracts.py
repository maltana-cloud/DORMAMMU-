...
class CapabilityRequirement:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.purpose.strip(): raise ValueError("purpose is required")
...
class ResourceDescriptor:
    ...
    metadata: Mapping[str, str] = field(default_factory=dict)
    def __post_init__(self) -> None:
        if not self.resource_id.strip() or not self.name.strip(): raise ValueError("resource_id and name are required")
        if self.cost < 0: raise ValueError("cost must be non-negative")
