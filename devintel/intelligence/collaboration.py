...
class HumanCollaborationEngine:
    ...
    def create_request(self, scope_id: str, objective: str, *, context: str = "", evidence: tuple[str, ...] = ()) -> CollaborationRequest:
        ...
        payload = "\x1f".join((normalized_scope, normalized_objective, context.strip(), *evidence))
        request_id = "collab-" + sha256(payload.encode("utf-8")).hexdigest()
        return CollaborationRequest(request_id, normalized_scope, normalized_objective, context.strip(), tuple(evidence))

    @staticmethod
    def respond(request: CollaborationRequest, responder_id: str, message: str, *, accepted: bool, reason: str = "") -> CollaborationResponse:
    ...
    @staticmethod
    def apply_response(request: CollaborationRequest, response: CollaborationResponse) -> CollaborationRequest:
        ...
        return CollaborationRequest(request.request_id, request.scope_id, request.objective, request.context, request.evidence, status)
